...
export type JettonMinterBCContent = {
    type: 0 | 1,
    uri: string
};

export type JettonMinterBCConfig = { admin: Address; content: Cell; wallet_code: Cell };

export function JettonMinterBCConfigToCell(config: JettonMinterBCConfig): Cell {
    return beginCell()
        .storeCoins(0)
...
export function jettonContentToCell(content: JettonMinterBCContent) {
    return beginCell()
...
export class JettonMinterBC implements Contract {
    constructor(readonly address: Address, readonly init?: { code: Cell; data: Cell }) {
    }

    static createFromAddress(address: Address) {
        return new JettonMinterBC(address);
    }

    static createFromConfig(config: JettonMinterBCConfig, code: Cell, workchain = 0) {
        const data = JettonMinterBCConfigToCell(config);
        const init = {code, data};
        return new JettonMinterBC(contractAddress(workchain, init), init);
    }
    ...
    */
    ...
    async sendDiscovery(provider: ContractProvider, via: Sender, owner: Address, include_address: boolean, value: bigint = toNano('0.1')) {
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: JettonMinterBC.discoveryMessage(owner, include_address),
            value: value,
        });
    }
    ...
    async sendChangeAdmin(provider: ContractProvider, via: Sender, newOwner: Address) {
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: JettonMinterBC.changeAdminMessage(newOwner),
            value: toNano("0.05"),
        });
    }
    ...
    async sendChangeContent(provider: ContractProvider, via: Sender, content: Cell) {
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: JettonMinterBC.changeContentMessage(content),
            value: toNano("0.05"),
        });
    }
